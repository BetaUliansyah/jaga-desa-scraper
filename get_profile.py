...
kode_desa_list = desa_df['id']

# Function to fetch data for a single kode_desa
def fetch_data(kode_desa):
    try:
        response = s.get(f'https://jaga.id/api/v5/desa/summary/{kode_desa}/{tahun}?anggaranKumulatif=1', verify=False)
        response_json = response.json()
        return response_json['data']['anggaran']  # Return dictionary
...
data_df.to_csv('data-anggaran-desa-concurrent.csv', index=False)
